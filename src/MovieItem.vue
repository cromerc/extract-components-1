...
const props = defineProps(['movie', 'id']);
      ...
      <div class="movie-item-rating-wrapper">
        ...
        <div class="movie-item-star-icon-wrapper">
          <button v-for="star in 5" :key="star" class="movie-item-star-icon-button" :class="[
            ...
            ]" :disabled="star === movie.rating" @click="$emit('update:rating', id, star)">
            <StarIcon class="movie-item-star-icon" />
          </button>
        </div>

        <div class="movie-item-actions-list-wrapper">
          <button class="movie-item-action-edit-button" @click="$emit('edit', id)">
            <PencilIcon class="w-4 h-4" />
          </button>
          <button class="movie-item-action-remove-button" @click="$emit('remove', id)">
