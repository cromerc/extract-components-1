...
function updateRating(id, rating) {
  movies.value[id].rating = rating;
}
function removeMovie(id) {
  movies.value = movies.value.filter((movie, index) => index !== id);
}
function editMovie(id) {
  const movie = movies.value[id];
...
<template>
  <div class="app">
    ...
    <div class="movie-list">
      <MovieItem v-for="(movie, movieIndex) in movies" :movie="movie" :id="movieIndex" :key="movie.id"
        @update:rating="updateRating" @edit="editMovie" @remove="removeMovie" />
    </div>
  </div>
</template>
